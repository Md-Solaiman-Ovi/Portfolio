      ...
      {links.map((link, index) => {
        ...
        return (
          <a
            href={link.path}
            key={index}
            className={`${link.path === pathname.hash && "bg-teal-700 text-white hover:border-none hover:text-white"} flex size-14 items-center justify-center rounded-full font-bold hover:border-[1px] hover:border-teal-500 hover:text-teal-500`}
          >
            {link.name}
          </a>
        );
